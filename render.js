...
function buildUpperFunnel() {
  ...
  const maxVal = Math.max(
    current.userViews,
    current.signups,
    cumulative.totalRegisteredUsers
  );
  const rows = [
    ...
    return `
      <div class="funnel-row">
        ...
        <div class="funnel-bar-bg">
          <div class="funnel-bar-fill ${r.cls}" style="width:${width}">
            ${fmtNumber(r.value)}
          </div>
        </div>
  ...
  setText(
    "acquisitionInsight",
    `Referrals (organic) drives ${share.toFixed(1)}% of current-week signups with ${referral.cvr.toFixed(1)}% conversion. Overall cumulative register rate is ${fmtPctFromRatio(registerRate, 2)}.`
  );
  setText(
    "channelInsight",
    `Current grouped mix is dominated by Referral (${(ratio(1024, 1090) * 100).toFixed(1)}% of signups). Content/SEO is small but measurable; Paid Social is currently empty in the available data.`
  );
  new Chart(document.getElementById("signupsByChannelChart"), {
    ...
    options: {
      ...
      scales: {
        x: { stacked: true, grid: { display: false }, ticks: { color: "#627490", font: { size: 14 } } },
        y: { stacked: true, beginAtZero: true, ticks: { color: "#627490" }, grid: { color: "#e5ebf5" } }
      }
    }
  });
}
...
function buildRetention() {
  ...
  setText("retentionD7", fmtPctValue(data.retention.d7 * 100, 0));
  setText("retentionD30", fmtPctValue(data.retention.d30 * 100, 0));
  const d1Delta = (data.retention.d1 - data.retention.previousD1) * 100;
  const el = document.getElementById("retentionD1Delta");
  setPill(el, d1Delta, true, "pp", 1);

  // 红框：补全 Retention Health 数值
  const currRate = 5.65;
  const nurrRate = 4.55;
  ...
  setText("new-users", fmtNumber(data.engagement.dauTrend[4].newUsers));
  ...
  new Chart(document.getElementById("retentionChart"), {
    ...
    options: {
      ...
      scales: {
        x: { ticks: { color: "#627490" }, grid: { display: false } },
        y: { beginAtZero: true, max: 100, ticks: { color: "#627490" } }
      }
    }
  });
}
...
function buildEngagement() {
  ...
  new Chart(document.getElementById("dauTrendChart"), {
    ...
    data: {
      ...
      datasets: [
        {
          label: "Returning DAU",
          data: data.engagement.dauTrend.map(d => d.returningUsers),
          borderColor: "#3559a1",
          backgroundColor: "rgba(53,89,161,0.08)",
          pointRadius: 4,
          tension: 0.25
        },
        {
          label: "New DAU",
          data: data.engagement.dauTrend.map(d => d.newUsers),
          borderColor: "#1fb980",
          backgroundColor: "rgba(31,185,128,0.08)",
          pointRadius: 4,
          tension: 0.25
        }
      ]
    ...
    options: {
      ...
      scales: {
        x: { ticks: { color: "#627490" }, grid: { display: false } },
        y: { beginAtZero: true, ticks: { color: "#627490" }, grid: { color: "#e5ebf5" } }
      }
    }
  });
  ...
  new Chart(document.getElementById("userMsgDistChart"), {
    ...
    data: {
      ...
      datasets: [{
        data: data.engagement.userMessageDistribution.values,
        backgroundColor: ["#4d77c3", "#6d28d9", "#2a9d8f", "#db2777", "#ef4444", "#8b5a2b"]
      }]
    },
    options: {
      responsive: true,
  ...
  new Chart(document.getElementById("sessionDepthChart"), {
    ...
    data: {
      ...
      datasets: [{
        data: data.engagement.sessionDepthDistribution.values,
        backgroundColor: ["#4d77c3", "#6d28d9", "#2a9d8f", "#db2777", "#ef4444", "#8b5a2b"]
      }]
    },
    options: {
  ...
  new Chart(document.getElementById("modeMixChart"), {
    ...
    data: {
      ...
      datasets: [{
        data: data.engagement.modeMix.values,
        backgroundColor: ["#3559a1", "#f59e0b", "#1fb980"],
        borderColor: "#ffffff",
        borderWidth: 2
      }]
    },
  ...
  new Chart(document.getElementById("featureAdoptionChart"), {
    ...
    data: {
      ...
      datasets: [{
        data: featurePct,
        backgroundColor: ["#3559a1", "#4d77c3", "#87b0e3", "#1fb980", "#f59e0b", "#f97316"]
      }]
    },
    options: {
      indexAxis: "y",
      ...
      scales: {
        ...
        y: { ticks: { color: "#627490" }, grid: { display: false } }
      }
    }
  });
}
...
function buildMonetization() {
  ...
  new Chart(document.getElementById("revenueTierChart"), {
    ...
    data: {
      ...
      datasets: [{
        data: m.mockRevenueByTier.map(x => x.value),
        backgroundColor: ["#3559a1", "#4d77c3", "#9ec8f5", "#c7d0dd"],
        borderColor: "#ffffff",
        borderWidth: 2
      }]
    },
